"""
Crexi scraper — backup source for NYC commercial listings 90+ DOM.

Same pattern as the LoopNet scraper; uses Playwright headless.
"""
...
CREXI_SEARCH_URL = (
    "https://www.crexi.com/properties?state=NY&city=New%20York&"
    "listingTypes=sale&daysOnMarket=90"
)
...
async def scrape_crexi(db: Session) -> list[Lead]:
    """Scrape Crexi for NYC commercial listings ≥90 DOM."""
    try:
    ...
    headless = os.environ.get("CREXI_HEADLESS", "true").lower() != "false"
    new_leads = []
    ...
    async with async_playwright() as pw:
        ...
        try:
            await page.goto(CREXI_SEARCH_URL, timeout=30_000)
            await page.wait_for_selector("[class*='property-card'], [class*='PropertyCard']", timeout=15_000)

            cards = await page.query_selector_all("[class*='property-card'], [class*='PropertyCard']")
            logger.info("Crexi: found %d listing cards", len(cards))
            ...
            for card in cards:
                try:
                    ...
                    type_el = await card.query_selector("[class*='property-type'], [class*='PropertyType']")
                    raw["property_type"] = (await type_el.inner_text()).strip() if type_el else None

                    dom_el = await card.query_selector("[class*='days'], [class*='Days']")
                    raw["dom"] = (await dom_el.inner_text()).strip() if dom_el else None

                    cap_el = await card.query_selector("[class*='cap'], [class*='Cap']")
                    ...
                    external_id = f"crexi:{hash(address + (raw.get('url') or ''))}"
                    ...
                    if existing:
                        if price and existing.asking_price and price != existing.asking_price:
                            db.add(PriceHistory(lead=existing, price=price, source="crexi_rescrape"))
                    ...
                    db.add(lead)
                    if price:
                        db.add(PriceHistory(lead=lead, price=price, source="crexi"))
                    db.commit()
                    db.refresh(lead)
                    new_leads.append(lead)
                    logger.info("Crexi lead: %s (score=%d, dom=%d)", address, bd.total, dom)

                except Exception as exc:
                    logger.warning("Error parsing Crexi card: %s", exc)

        except Exception as exc:
            logger.error("Crexi scrape failed: %s", exc)
        finally:
            await browser.close()
